<?php

// Average yards to go per down
// {  
//   "aggs": {
//     "down": {
//       "terms": { "field": "down" },
//       "aggs" : {
//         "yardstogo": {
//           avg: { "field": "ydstogo"}
//         }
//       }
//     }
//   }
// }

// Yards per drive, per team, ordered by total yards in the season
// {    
//   "aggs": {
//     "quarter": {
//       "terms": { "field": "qtr", "size": 100, "order": {"yards.sum" : "desc"} },
//         "aggs": {
//           "yards": {
//             "stats": {"field": "yards"}
//         }
//       }
//     }
//   }
// }

// Tom Brady's passing yard statistics, broken down by quarter, to anyone except Amendola
// {
// "query": {
//  		"bool": {
//  			"must": [
//  				{"match": {"players.playerName": "T.Brady"}},
//  				{"match": {"desc": "pass"}}
//  			],
// 			"must_not": [
// 				{"match": {"players.playerName": "D.Amendola"}}
// 			]
//  		}
//  	},
// 	"aggs": {
// 		"quarter": {
// 			"terms": { "field": "qtr", "size": 5, "order": {"yards.sum" : "desc"} },
// 				"aggs": {
// 					"yards": {
// 						"stats": {"field": "yards"}
// 					}
// 				}
// 			}
// 		}
// 	}
// }

// NE top recievers who didn't drop the ball
// {
// "query": {
//  		"bool": {
//  			"must": [
//  				{"match": {"players.playerName": "T.Brady"}},
//  				{"match": {"desc": "pass"}}
//  			],
// 			"must_not": [
// 				{"match": {"desc": "incomplete"}}
// 			]
//  		}
//  	},
// "aggs": {
//      "quarter": {
//        "terms": { "field": "players.playerName", "size": 5, "order": {"yards.count" : "desc"} },
//          "aggs": {
//            "yards": {
//              "stats": {"field": "yards"}
//          }
//        }
//      }
//    }
//  }
// } 

$endpoint = 'opportunity.local:9200';
$ch = curl_init();

function get_data($ch, $game) {
	$url = "http://www.nfl.com/liveupdate/game-center/".$game."/".$game."_gtd.json";
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
	curl_setopt($ch, CURLOPT_POSTFIELDS, null);
	curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 20);
	$data = curl_exec($ch);
	return $data;
}

function set_mapping($ch, $game) {
	global $endpoint;
	$map = [
		$game => [
			'_timestamp': {
	            'enabled': 'true'
	        },
			'properties' => [
				'desc' 		=> ['type' => 'string', 'analyzer' => 'english'],
				'down' 		=> ['type' => 'long'],
				'drive' 	=> ['type' => 'long'],
				'players'	=> [
					'type' => 'object',
					'properties' => [
						'clubcode' 	=> ['type' => 'string'],
						'playerName'=> ['type' => 'string'],
						'sequence' 	=> ['type' => 'long'],
						'statId' 	=> ['type' => 'long'],
						'yards' 	=> ['type' => 'long'],
					]
				],
				'posteam' 	=> ['type' => 'string'],
				'qtr' 		=> ['type' => 'long'],
				'sp' 		=> ['type' => 'long'],
				'time' 		=> ['type' => 'string'],
				'ydsnet'	=> ['type' => 'long'],
				'ydstogo'	=> ['type' => 'long'],
				'yrdln'		=> ['type' => 'string'],
			]
		]
	];

	$url = "http://".$endpoint."/nfl/".$game."/_mapping";
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 20);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
	curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($map));
	return curl_exec($ch);
}

function post_data($ch, $game, $drive, $play, $data) {
	global $endpoint;

	$url = "http://".$endpoint."/nfl/".$game."/".$play;
	$data->drive = $drive;
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 20);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
	curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($data));
	return curl_exec($ch);
}

$games = [
	// "2014080300",
	// "2014080700",
	// "2014080701",
	// "2014080702",
	// "2014080703",
	// "2014080704",
	// "2014080705",
	// "2014080800",
	// "2014080801",
	// "2014080802",
	// "2014080803",
	// "2014080804",
	// "2014080805",
	// "2014080900",
	// "2014080901",
	// "2014080902",
	// "2014080903",
	// "2014081400",
	// "2014081501",
	// "2014081502",
	// "2014081503",
	// "2014081504",
	// "2014081600",
	// "2014081601",
	// "2014081602",
	// "2014081603",
	// "2014081604",
	// "2014081605",
	// "2014081606",
	// "2014081607",
	// "2014081700",
	// "2014081701",
	// "2014081800",
	// "2014082100",
	// "2014082200",
	// "2014082201",
	// "2014082202",
	// "2014082203",
	// "2014082204",
	// "2014082300",
	// "2014082301",
	// "2014082302",
	// "2014082303",
	// "2014082304",
	// "2014082305",
	// "2014082306",
	// "2014082307",
	// "2014082400",
	// "2014082401",
	// "2014082800",
	// "2014082801",
	// "2014082802",
	// "2014082803",
	// "2014082804",
	// "2014082805",
	// "2014082806",
	// "2014082807",
	// "2014082808",
	// "2014082809",
	// "2014082810",
	// "2014082811",
	// "2014082812",
	// "2014082813",
	// "2014082814",
	// "2014082815",
	// "2014090400",
	// "2014090700",
	// "2014090701",
	// "2014090702",
	// "2014090703",
	// "2014090704",
	// "2014090705",
	// "2014090706",
	// "2014090707",
	// "2014090708",
	// "2014090709",
	// "2014090710",
	// "2014090711",
	// "2014090712",
	// "2014090800",
	// "2014090801",
	// "2014091100",
	// "2014091400",
	// "2014091401",
	// "2014091402",
	// "2014091403",
	// "2014091404",
	// "2014091405",
	// "2014091406",
	// "2014091407",
	// "2014091408",
	// "2014091409",
	// "2014091410",
	// "2014091411",
	// "2014091412",
	// "2014091413",
	// "2014091500",
	// "2014091800",
	// "2014092100",
	// "2014092101",
	// "2014092102",
	// "2014092103",
	// "2014092104",
	// "2014092105",
	// "2014092106",
	// "2014092107",
	// "2014092108",
	// "2014092109",
	// "2014092110",
	// "2014092111",
	// "2014092112",
	// "2014092113",
	// "2014092200",
	// "2014092500",
	// "2014092800",
	// "2014092801",
	// "2014092802",
	// "2014092803",
	// "2014092804",
	// "2014092805",
	// "2014092806",
	// "2014092807",
	// "2014092808",
	// "2014092809",
	// "2014092810",
	// "2014092900",
	// "2014100200",
	// "2014100500",
	// "2014100501",
	// "2014100502",
	// "2014100503",
	// "2014100504",
	// "2014100505",
	// "2014100506",
	// "2014100507",
	// "2014100508",
	// "2014100509",
	// "2014100510",
	// "2014100511",
	// "2014100512",
	// "2014100600",
	// "2014100900",
	// "2014101200",
	// "2014101201",
	// "2014101202",
	// "2014101203",
	// "2014101204",
	// "2014101205",
	// "2014101206",
	// "2014101207",
	// "2014101208",
	// "2014101209",
	// "2014101210",
	// "2014101211",
	// "2014101212",
	// "2014101300",
	// "2014101600",
	// "2014101900",
	// "2014101901",
	// "2014101902",
	// "2014101903",
	// "2014101904",
	// "2014101905",
	// "2014101906",
	// "2014101907",
	// "2014101908",
	// "2014101909",
	// "2014101910",
	// "2014101911",
	// "2014101912",
	// "2014102000",
	// "2014102300",
	// "2014102600",
	// "2014102601",
	// "2014102602",
	// "2014102603",
	// "2014102604",
	// "2014102605",
	// "2014102606",
	// "2014102607",
	// "2014102608",
	// "2014102609",
	// "2014102610",
	// "2014102611",
	// "2014102612",
	// "2014102700",
	// "2014103000",
	// "2014110200",
	// "2014110201",
	// "2014110202",
	// "2014110203",
	// "2014110204",
	// "2014110205",
	// "2014110206",
	// "2014110207",
	// "2014110208",
	// "2014110209",
	// "2014110210",
	// "2014110300",
	// "2014110600",
	// "2014110900",
	// "2014110901",
	// "2014110902",
	// "2014110903",
	// "2014110904",
	// "2014110905",
	// "2014110906",
	// "2014110907",
	// "2014110908",
	// "2014110909",
	// "2014110910",
	// "2014111000",
	// "2014111300",
	// "2014111600",
	// "2014111601",
	// "2014111602",
	// "2014111603",
	// "2014111604",
	// "2014111605",
	// "2014111606",
	// "2014111607",
	// "2014111608",
	// "2014111609",
	// "2014111610",
	// "2014111611",
	// "2014111700",
	// "2014112000",
	// "2014112300",
	// "2014112301",
	// "2014112302",
	// "2014112303",
	// "2014112304",
	// "2014112305",
	// "2014112306",
	// "2014112307",
	// "2014112308",
	// "2014112309",
	// "2014112310",
	// "2014112311",
	// "2014112312",
	// "2014112400",
	// "2014112700",
	// "2014112701",
	// "2014112702",
	// "2014113000",
	// "2014113001",
	// "2014113002",
	// "2014113003",
	// "2014113004",
	// "2014113005",
	// "2014113006",
	// "2014113007",
	// "2014113008",
	// "2014113009",
	// "2014113010",
	// "2014113011",
	"2014120100",
	"2014120400",
	"2014120700",
	"2014120701",
	"2014120702",
	"2014120703",
	"2014120704",
	"2014120705",
	"2014120706",
	"2014120707",
	"2014120708",
	"2014120709",
	"2014120710",
	"2014120711",
	"2014120712",
	"2014120713",
	"2014120800",
	"2014121100",
	"2014121400",
	"2014121401",
	"2014121402",
	"2014121403",
	"2014121404",
	"2014121405",
	"2014121406",
	"2014121407",
	"2014121408",
	"2014121409",
	"2014121410",
	"2014121411",
	"2014121412",
	"2014121413",
	"2014121500",
	"2014121800",
	"2014122000",
	"2014122001",
	"2014122100",
	"2014122101",
	"2014122102",
	"2014122103",
	"2014122104",
	"2014122105",
	"2014122106",
	"2014122107",
	"2014122108",
	"2014122109",
	"2014122110",
	"2014122111",
	"2014122200",
	"2014122800",
	"2014122801",
	"2014122802",
	"2014122803",
	"2014122804",
	"2014122805",
	"2014122806",
	"2014122807",
	"2014122808",
	"2014122809",
	"2014122810",
	"2014122811",
	"2014122812",
	"2014122813",
	"2014122814",
	"2014122815",
	"2015010300",
	"2015010301",
	"2015010400",
	"2015010401",
	"2015011000",
	"2015011001",
	"2015011100",
	"2015011101",
	"2015011800",
	"2015011801",
	"2015020100",
];

foreach ($games as $key => $game) {
	echo "Fetching game #".($key+1)."\n";

	$data = json_decode(get_data($ch, $game));
	set_mapping($ch, $game);
	
	foreach ($data->$game->drives as $driveKey => $drive) {
		if($driveKey == 'crntdrv')
			continue;
		
		echo "\tProcessing drive #".$driveKey."\n";

		foreach ($drive->plays as $playKey => $play) {

			$playerArray = [];
			foreach ($play->players as $playerKey => $player)
				$playerArray[] = $player;
			
			$play->players = $playerArray;
			post_data($ch, $game, $driveKey, $playKey, $play);
		}
	}
	
	echo "\tGame Complete\n\n";
}


curl_close($ch);